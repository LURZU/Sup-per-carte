...
class ProgrammeQuotidienController extends Controller
{
    ...
    public function selectParametersMyCard(): View | RedirectResponse{
        if(auth()->user()->hasRole('enseignant') || auth()->user()->hasRole('admin')) {
            return redirect()->route('card.profcard');
        }
        $user = auth()->user();
    ...
    public function startProgramMyCard(QuizzSelectRequest $request): View {
        if (auth()->user()->hasRole('etudiant')) {
            $user = auth()->user();
            $cards = Card::where('matiere_id', $request->input('matiere_id'))->where('user_id', auth()->user()->id)->get();
            $card = new Card();
            $chapitre = new Chapitre();
            $cardLevels = new CardLevel();
            $matiere = new Matiere();
